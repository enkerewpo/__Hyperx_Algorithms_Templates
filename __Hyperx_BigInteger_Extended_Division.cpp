...
public:
    ...
    void read() {
        char s[N];
        scanf("%s", s + 1);
        int len = strlen(s + 1), st = 1; if (s[1] == '-') st = 2, minus = true;
        int p = 1, tmp = 0, y;
        a[0] = 1;
        for (int i = len; i >= st; i--) {
            y = (s[i] - '0') % 10, tmp += y * Pow(10, p - 1), p %= 8;
            if (p == 0) {
                a[a[0]] = tmp;
                tmp = 0;
                a[0]++;
    ...
    BigInt operator * (int rhs) {
        ...
        for (int i = 1; i <= a[0]; i++) {
            c.a[i] += a[i] * rhs;
            if (c.a[i] >= zip) c.a[i + 1] += c.a[i] / zip, c.a[i] %= zip;
        }
        c.a[0] = a[0];
    ...
    BigInt operator / (int rhs) {
        BigInt c;
        LL x = 0;
        for (int i = a[0]; i; i--) x = x * zip + a[i], c.a[i] = x / rhs, x = x % rhs;
        c.a[0] = a[0];
        if (c.a[0] && !c.a[c.a[0]]) c.a[0]--;
        return c;
    }
    LL operator % (int rhs) {
        BigInt c;
